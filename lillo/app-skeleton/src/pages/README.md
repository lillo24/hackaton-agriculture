...
- `DashboardPage.jsx` (`DRAFT`) - home route with a visual alert-first summary line, flatter containers, water/soil widgets, and a placeholder AI assistant seed with a typed mock reply.
...
- `pages.css` (`DRAFT`) - page-level layout and route-specific styling for the active page modules above, including the flatter dashboard card treatment, visual alert chips, assistant seed chat styling, and profile composition.
...
- The alert filter strip is sticky at the top of the scrollable page area, so controls remain visible while scanning long alert feeds.
