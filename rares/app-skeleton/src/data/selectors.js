...
export function buildAlertsForFarm({
  ...
  return templates
    ...
    .map((template) => {
      ...
      const sourceIds = template.sourceIdsByFarm?.[farmId] ?? template.sourceIds;

      if (!field) {
        throw new Error(`Alert "${template.id}" references unknown field "${fieldId}" for farm "${farmId}".`);
      }

      const sourceObjects = sourceIds.map((sourceId) => {
      ...
      return {
        ...
        integratedSummary: template.integratedSummaries?.[farmId] ?? readFarmCopy(template.reasons, farmId, template.id, 'reason'),
        riskScore: template.riskScores?.[farmId] ?? null,
