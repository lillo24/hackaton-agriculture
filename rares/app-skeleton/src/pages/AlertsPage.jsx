...
function AlertsPage({
  ...
  const filteredAlerts = useMemo(
    () => {
      const liveAlerts = rankedAlerts.filter((alert) => {
      ...
      return [...liveAlerts, ...visibleHistoryAlerts];
    },
    ...
    <div className="page alerts-page">
      ...
      {!isLoading && alerts.length > 0 && filteredAlerts.length === 0 ? (
        <SectionCard subtitle="No alerts match the current filter selection." title="No results for filters">
          <p className="detail-text">Try widening severity or source to recover the full operational feed.</p>
          {hasActiveFilters ? (
            <button className="inline-action" onClick={clearFilters} type="button">
              Clear filters
            </button>
