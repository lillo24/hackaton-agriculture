...
const customSideIconsByAlertId = {
  'humidity-spike': [
    { type: 'weather', label: 'Rainfall and forecast moisture' },
    { type: 'humidity', label: 'Leaf wetness and disease pressure' },
  ],
  'irrigation-drift': [
    { type: 'heat', label: 'Heat and high temperature risk' },
    { type: 'sun', label: 'Sun exposure and radiation risk' },
  ],
};
...
function AlertIconGlyph({ type }) {
  ...
  if (type === 'heat') {
    return (
      <svg aria-hidden="true" viewBox="0 0 24 24">
        <path d="M12.1 4.5c1.7 2 2.8 3.7 2.8 5.4 0 1.2-.5 2.1-1.5 2.9.2-1.2-.2-2.2-1.1-3.1-2 1.3-3.3 3.2-3.3 5.6 0 2.5 1.9 4.2 4.3 4.2 2.8 0 4.9-2.1 4.9-5.1 0-3.1-2.2-6.4-6.1-9.9z" />
      </svg>
    );
  }
...
function pickSideIcons(alert) {
  const customIcons = customSideIconsByAlertId[alert.id];
  ...
  return [signalIcon, { ...problemIcon, label: `Problem type: ${problemIcon.label}` }];
}
...
function AlertListItem({
  ...
  const sideIcons = pickSideIcons(alert);
      ...
      <div className="alert-list-item__layout">
        ...
        <div className="alert-list-item__side">
          <div className="alert-list-item__icons alert-list-item__icons--large" role="presentation">
            {sideIcons.map((icon) => (
              <AlertIcon key={`${alert.id}-${icon.type}`} label={icon.label} type={icon.type} />
            ))}
          </div>
        </div>
      </div>
